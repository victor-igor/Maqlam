-- Migration: Update Campaigns Schema for Hybrid Support (Official + Unofficial)
-- Adds provider fields and dispatch logic

-- 1. Update campaigns table
ALTER TABLE public.campaigns
ADD COLUMN IF NOT EXISTS provider TEXT CHECK (provider IN ('official', 'unofficial')) DEFAULT 'unofficial',
ADD COLUMN IF NOT EXISTS template_name TEXT,
ADD COLUMN IF NOT EXISTS template_language TEXT,
ADD COLUMN IF NOT EXISTS template_text TEXT;

-- ensure recurrence_rule is reliable (it usually is jsonb, but just in case of type drift)
-- ALTER TABLE public.campaigns ALTER COLUMN recurrence_rule TYPE JSONB USING recurrence_rule::JSONB;

-- 2. Create Log Table if not exists (for dispatch history)
CREATE TABLE IF NOT EXISTS public.campaign_triggers_log (
    ...
    phone TEXT, -- useful for individual message tracking if needed
    contact_id UUID -- optional linkage to contact
);
...
-- 3. Trigger Function for INSTANT Campaigns
    ...
    IF NEW.type = 'instant' AND NEW.status = 'pending' THEN
        BEGIN
            ...
            ELSIF NEW.provider = 'official' THEN
                -- Fire Webhook to Gateway (which handles Official API logic too, typically) 
                -- OR direct to Meta if we were doing it purely in DB, but the existing pattern uses the gateway/webhook.
                -- *Assuming* the gateway handles both or we are just sending the command to an endpoint that routes it.
                -- Based on Campos Joias implementation, it sends to `gateway_url` which likely handles the actual dispatch logic.
                ...
                SELECT net.http_post(
                    ...
                    body := jsonb_build_object(
                        ...
                        'meta_token', meta_token,         -- passing creds to worker possibly
...
-- 5. CRON Function (Process Queue) for Scheduled/Recurring
...
BEGIN
    ...
    -- ========================================
    -- Similar logic to original, just ensuring props are passed
    FOR campaign_record IN
        SELECT 
            *,
            CASE 
